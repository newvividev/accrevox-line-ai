...
- รอบนี้รองรับ `AI fallback` เฉพาะตอน parser ปกติอ่านคำสั่งไม่ออก
- ถ้าข้อความยังอยู่ในรูปแบบ `ลูกค้า=... วันที่=... รายการ=...` ระบบจะไม่เรียก AI และไม่หักเครดิต
- ถ้าผู้ใช้พิมพ์ภาษาคน เช่น `ช่วยออกใบแจ้งหนี้ให้บริษัท ABC ปากกา 10 ด้าม ด้ามละ 20 ครบกำหนดพรุ่งนี้` ระบบจะลองเรียก AI เพื่อแปลงเป็น JSON
- ตอนนี้เลือก provider ได้ทั้ง `openai` และ `ollama`
- ถ้าต้องการเริ่มแบบถูกที่สุด แนะนำ `ollama` บนเครื่องก่อน
- ค่าเริ่มต้นของโมเดลฝั่ง OpenAI ตั้งไว้เป็น `gpt-5-nano` เพื่อกดต้นทุนก่อน แล้วค่อยขยับเป็นรุ่นที่แม่นขึ้นภายหลังได้
...
- ถ้า app รันใน Docker แต่ Ollama รันบนเครื่อง host ให้ใช้ `host.docker.internal`
- ถ้ารัน app ตรงบนเครื่องเดียวกันไม่ผ่าน Docker ใช้ `http://127.0.0.1:11434`
